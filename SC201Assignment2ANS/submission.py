...
def learnPredictor(
    ...
    for epoch in range(numEpochs):
        ...
        print(
            f"Training Error: ({epoch} epoch): {evaluatePredictor(trainExamples, predictor)}"
        )
        print(
            f"Validation Error: ({epoch} epoch): {evaluatePredictor(validationExamples, predictor)}"
        )
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        and value is its occurrence (randomize).
...
def extractCharacterFeatures(n: int) -> Callable[[str], FeatureVector]:
    ...
    def extract(x: str) -> Dict[str, int]:
        ...
        x_without_spaces = "".join(x.split())
        ans_d = defaultdict(int)
        for i in range(len(x_without_spaces) - n + 1):
            ans_d[x_without_spaces[i : i + n]] += 1
        return ans_d
        # END_YOUR_CODE
