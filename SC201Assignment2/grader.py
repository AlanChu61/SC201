...
numEpochs=70,
Official: train error = 0.014631401238041642, validation error = 0.26139561057962857
numEpochs=80
Official: train error = 0.012380416432189083, validation error = 0.263083849184018
numEpochs=90
Official: train error = 0.009566685424873381, validation error = 0.2658975801913337
numEpochs=12
Official: train error = 0.004783342712436691, validation error = 0.2658975801913337
The train error decreas with the increasement of numEpochs, but the validation error increases with the increasement of numEpochs.
"""
...
grader.add_basic_part(
    "3a-0", test3a0, max_points=10, max_seconds=1, description="basic test"
)
...
grader.add_basic_part(
    ...
    max_points=10,
    max_seconds=1,
    description="basic sanity check for adding scale * d2 on d1",
)
...
grader.add_basic_part(
    ...
    max_points=10,
    max_seconds=1,
    description="basic sanity check for the answer of dot product of 2 sparse vectors d1 and d2",
)
...
def test4a0():
    ...
    weights = submission.learnPredictor(
        trainExamples, testExamples, featureExtractor, numEpochs=20, alpha=0.01
    )
...
def test4a2():
    ...
    weights = submission.learnPredictor(
        trainExamples, validationExamples, featureExtractor, numEpochs=40, alpha=0.01
    )
...
Official: train error = 0.0, validation error = 0.27771525042205963

Yes, there is a difference between using extractCharacterFeatures and extractWordFeatures.
The extractCharacterFeatures function returns a function that takes a string |x| and returns a sparse feature vector consisting of all n-grams of |x| without spaces mapped to their n-gram counts.
The extractWordFeatures function returns a sparse feature vector of unigram word features.
"""
############################################################
...
def test5a0():
    ...
    for data_pt in data:
        grader.require_is_equal(
            (util.dotProduct(data_pt[0], weights) >= 0), (data_pt[1] == 1)
        )
...
grader.add_basic_part(
    ...
    max_points=5,
    max_seconds=2,
    description="test correct generation of random dataset labels",
)
...
grader.add_basic_part(
    ...
    max_points=5,
    max_seconds=2,
    description="test that the randomly generated example actually coincides with the given weights",
)
